...
GCSQL_POSTGRES_SERVER_CA_FILE = os.environ.get(
    "GCSQL_POSTGRES_SERVER_CA_FILE", ".key/postgres-server-ca.pem"
)
...
GCSQL_MYSQL_CLIENT_CERT_FILE = os.environ.get(
    "GCSQL_MYSQL_CLIENT_CERT_FILE", ".key/mysql-client-cert.pem"
)
GCSQL_MYSQL_CLIENT_KEY_FILE = os.environ.get(
    "GCSQL_MYSQL_CLIENT_KEY_FILE", ".key/mysql-client-key.pem"
)
GCSQL_MYSQL_SERVER_CA_FILE = os.environ.get(
    "GCSQL_MYSQL_SERVER_CA_FILE", ".key/mysql-server-ca.pem"
)
...
try:
    sql_proxy_binary_path = (
        subprocess.check_output(["which", "cloud_sql_proxy"]).decode("utf-8").rstrip()
    )
...
with models.DAG(
    dag_id="example_gcp_sql_query", default_args=default_args, schedule_interval=None
) as dag:
    prev_task = None
