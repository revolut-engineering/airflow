...
if gcs_to_bq is not None:
    ...
    dag = models.DAG(
        dag_id="example_gcs_to_bq_operator", default_args=args, schedule_interval=None
    )

    create_test_dataset = bash_operator.BashOperator(
        task_id="create_airflow_test_dataset", bash_command="bq mk airflow_test", dag=dag
    )
    ...
    delete_test_dataset = bash_operator.BashOperator(
        task_id="delete_airflow_test_dataset", bash_command="bq rm -rf airflow_test", dag=dag
    )
