...
class CloudVisionHook(GoogleCloudBaseHook):
    ...
    _client = None
    product_name_determiner = NameDeterminer(
        "Product", "product_id", ProductSearchClient.product_path
    )
    ...
    def create_product_set(
        ...
        if not product_set_id:
            ...
            self.log.info(
                "Extracted autogenerated ProductSet ID from the response: %s", product_set_id
            )
    ...
    def get_product_set(
        ...
        response = client.get_product_set(
    ...
    @GoogleCloudBaseHook.fallback_to_default_project_id
    def get_product(
        self, location, product_id, project_id=None, retry=None, timeout=None, metadata=None
    ):
        """
        For the documentation see:
    ...
    def update_product(
        ...
        """
        client = self.get_conn()
        product = self.product_name_determiner.get_entity_with_name(
            product, product_id, location, project_id
        )
    ...
    def create_reference_image(
        ...
        parent = ProductSearchClient.product_path(
    ...
    def add_product_to_product_set(
        ...
        product_set_name = ProductSearchClient.product_set_path(
            project_id, location, product_set_id
        )

        self.log.info(
            "Add Product[name=%s] to Product Set[name=%s]", product_name, product_set_name
        )
    ...
    def remove_product_from_product_set(
        ...
        self.log.info(
            "Remove Product[name=%s] from Product Set[name=%s]", product_name, product_set_name
        )
    ...
    def _get_autogenerated_id(response):
        try:
            ...
            raise AirflowException(
                "Unable to get name from response... [{}]\n{}".format(response, e)
            )
        if "/" not in name:
