...
def get_ldap_connection(dn=None, password=None):
    ...
    server = Server(conf.get("ldap", "uri"), use_ssl=True, tls=tls_configuration)
...
class LdapUser(models.User):
    def __init__(self, user):
        ...
        conn = get_ldap_connection(
            conf.get("ldap", "bind_user"), conf.get("ldap", "bind_password")
        )
    ...
    def try_login(username, password):
        ...
        search_filter = "(&({0})({1}={2}))".format(
            conf.get("ldap", "user_filter"), conf.get("ldap", "user_name_attr"), username
        )
